...
def extract_features(image_number, slice_number):
    ...
    for s in scales:
        blurred_t1 = ndimage.gaussian_filter(t1, sigma=s)
        blurred_t1 = blurred_t1.flatten().T
        blurred_t1 = blurred_t1.reshape(-1, 1)

        blurred_t2 = ndimage.gaussian_filter(t2, sigma=s)
        blurred_t2 = blurred_t2.flatten().T
        blurred_t2 = blurred_t2.reshape(-1, 1)

        X = np.concatenate((X, blurred_t1, blurred_t2), axis=1)
        features += ('T1 blur ' + str(s),)
        features += ('T2 blur ' + str(s),)

    c, coord_im = seg.extract_coordinate_feature(t1)
...
def dice_overlap(true_labels, predicted_labels, smooth=1.):
    ...
    #!studentstart
    dice = (2*np.count_nonzero(t & p) + smooth)/(np.count_nonzero(t) + np.count_nonzero(p) + smooth)
    #!studentend
    #------------------------------------------------------------------#
    return dice
