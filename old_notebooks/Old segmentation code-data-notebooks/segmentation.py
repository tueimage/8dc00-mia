...
def extract_coordinate_feature(im):
    ...
    #!studentstart
    x_dist_from_center = x_coord-x_center;
    y_dist_from_center = y_coord-y_center;
    coord_im = np.sqrt(x_dist_from_center**2 + y_dist_from_center**2)
    #!studentend
    #------------------------------------------------------------------#
...
def segmentation_combined_atlas(train_labels_matrix, combining='mode'):
    ...
    #!studentstart
    # All atlases have to think it's brain: only for binary labels!
    elif combining == 'min':
        predicted_labels = np.min(predicted_labels, axis=1)
    # At least one atlas has to think it's brain: only for binary labels!
    elif combining == 'max':
